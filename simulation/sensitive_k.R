# Sensitivity analysis: grid size K (paper Appendix B.3)
...
run_simulation <- function(h_fun, n = 1000, R = 1000, n_cores = 13,
                           log_file = "scratch/k_log.txt") {

  pn_list  <- c(5 / n, 1 / n, 5 / (n * log(n)))
  true_qte <- calculate_true_qtes(h_fun, pn_list)$qtes
  k_list   <- seq(100, 2000, 100)
  ...
  res_list <- parallel::mclapply(1:R, function(r) {
    ...
    tryCatch({
      ...
      mat <- matrix(NA, nrow = length(pn_list), ncol = length(k_list),
                    dimnames = list(NULL, paste0("k_", k_list)))

      for (j in seq_along(pn_list)) {
        pn <- pn_list[j]
        for (i in seq_along(k_list)) {
          ipw <- tiee_cpp(Y, X, D, q = 1 - pn, u = 1 - n^0.65 / n,
                          R = 1e6,
                          tau = seq(0.001, 0.999, length.out = k_list[i]),
                          interval = c(0.5, 100), se = FALSE)
          mat[j, i] <- ipw$theta1 - ipw$theta0
        }
      }
      ...
      cat(sprintf("ERROR at iteration %d: %s\n", r, e$message),
          file = log_file, append = TRUE)
      matrix(NA, nrow = length(pn_list), ncol = length(k_list))
    })
  }, mc.cores = n_cores)

  res_array <- array(
    unlist(res_list),
    dim = c(length(pn_list), length(k_list), R),
    dimnames = list(
      paste0("pn=", round(pn_list, length(k_list))),
      paste0("k_", k_list), NULL
    )
  )
...
res_k <- run_simulation(h3.fun)
save.image("scratch/simu_k.Rdata")
